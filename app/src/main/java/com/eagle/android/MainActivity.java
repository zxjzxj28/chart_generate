...
public class MainActivity extends AppCompatActivity {
    ...
    // 图表标题和轴标签（论文示例：可隐藏轴标签，仅保留标题）
    private final String chartTitle = "2024年主要销售渠道业绩（亿元）";
    private final String xAxisLabel = "";
    private final String yAxisLabel = "";

    // 数据系列名称（图例显示）
    private final String[] seriesNames = {"销售额"};

    // 数据系列颜色 (学术论文配色 - 清爽低饱和度)
    private final int[] seriesColors = {
            Color.parseColor("#4A90E2")   // 静谧蓝
    };

    // 各系列数据值 - 单系列，列数需与xLabels长度一致
    private final float[][] seriesData = {
            {128f, 152f, 141f, 167f, 158f}
    };

    // Y轴最大值（设为0则自动计算）
    private final float yAxisMax = 180f;

    // 输出图片文件名前缀
    private final String outputFilePrefix = "single_series_bar_chart";
        ...
        for (int g = 0; g < groupCount; g++) {
            ...
            for (int s = 0; s < seriesCount; s++) {
                ...
                svg.append(String.format(
                        ...
                        barX + barWidth / 2f, valueY, seriesData[s][g]));
            }
